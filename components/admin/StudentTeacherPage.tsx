...
import { ADMIN_GRADES, ADMIN_SUBJECTS, INITIAL_TEACHERS } from '@/data/adminData';
import { AdminRole, AdminTeacher } from '@/types';
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                ...
                <select value={teacherSubject} onChange={(event) => setTeacherSubject(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900">
                  {ADMIN_SUBJECTS.map((subject) => <option key={subject.id} value={subject.name}>{subject.name}</option>)}
                </select>

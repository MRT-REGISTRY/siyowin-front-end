...
import { AdminStudent, AdminStudentMark } from '@/types';
...
const CSV_TEMPLATE = `${ADMIN_CSV_COLUMNS.join(',')}
${ADMIN_CSV_SAMPLE.join('\n')}`;
...
export default function MarksPage() {
  ...
  const classesForSelectedGrade = useMemo(() => getClassesForGrade(selectedGrade), [selectedGrade]);
  const subjectsForSelectedClass = useMemo(() => getSubjectsForClass(selectedClassId), [selectedClassId]);
  ...
  const currentSubject = ADMIN_SUBJECTS.find((subject) => subject.id === selectedSubjectId) ?? ADMIN_SUBJECTS[0];
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
          ...
          <div className="grid gap-4 border-b border-slate-200 px-6 py-6 sm:grid-cols-2 xl:grid-cols-4 sm:px-8">
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900">{getClassesForGrade(selectedGrade).find((item) => item.id === selectedClassId)?.label ?? 'No class selected'}</p>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
            ...
            <section className="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <select value={selectedGrade} onChange={(event) => setSelectedGrade(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900">
                  {ADMIN_GRADES.map((grade) => <option key={grade} value={grade}>{grade}</option>)}
                </select>
                ...
                <select value={selectedExamType} onChange={(event) => setSelectedExamType(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900">
                  {ADMIN_EXAM_TYPES.map((exam) => <option key={exam.id} value={exam.id}>{exam.label}</option>)}
                </select>
